...
describe('GovukValidators', () => {
  ...
  it('should override angular validators', () => {
    ...
    expect(GovukValidators.maxDecimalsValidator(5)(new FormControl(6.45346563))).toEqual({
      invalidDecimals: 'Enter a number up to 5 decimal places',
    });
    expect(GovukValidators.minMaxRangeNumberValidator(100, 999)(new FormControl(6))).toEqual({
      invalidLength: 'Enter a number between 100 and 999',
    });
  });
});
