...
export const CONTROL_ACTIVITY_ROUTES: Routes = [
  ...
  {
    path: ControlActivitiesWizardStep.VARIATION_REGULATOR_DECISION,
    title: controlActivitiesMap.variationRegulatorDecision.title,
    ...
    loadComponent: () =>
      import('@requests/tasks/emp-variation-regulator/subtasks/control-activities').then(
        (c) => c.ControlActivitiesVariationRegulatorDecisionComponent,
      ),
  },
  {
    path: ControlActivitiesWizardStep.QUALITY_ASSURANCE,
    title: controlActivitiesMap.qualityAssurance.title,
    canActivate: [canActivateControlActivitiesStep(ControlActivitiesWizardStep.VARIATION_REGULATOR_DECISION)],
    data: { breadcrumb: false },
    resolve: {
      backlink: backlinkResolver(ControlActivitiesWizardStep.SUMMARY, '../../'),
    },
