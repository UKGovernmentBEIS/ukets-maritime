...
@Injectable()
export class WaitForFollowUpApiService extends TaskApiService<WaitForFollowUpTaskPayload> {
  ...
  save(payload: WaitForFollowUpTaskPayload): Observable<WaitForFollowUpTaskPayload> {
    return this.service
      .processRequestTaskAction(
        this.createSaveAction({
          payloadType: payload.payloadType,
          dueDate: new Date(payload.followUpResponseExpirationDate).toISOString(),
        }),
      )
      .pipe(
        map(() => payload),
