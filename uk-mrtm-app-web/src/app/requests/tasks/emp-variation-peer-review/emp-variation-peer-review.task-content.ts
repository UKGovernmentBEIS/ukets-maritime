...
import { EmpVariationPeerReviewActionButtonsComponent } from '@requests/tasks/emp-variation-peer-review/components';
import { variationReviewDetailsSubtaskMap } from '@requests/tasks/emp-variation-review/subtasks/variation-details';
import { taskActionTypeToTitleMap } from '@shared/constants';

const routePrefix = 'emp-variation-peer-review';

export const empVariationPeerReviewTaskContent: RequestTaskPageContentFactory = () => {
  const store: RequestTaskStore = inject(RequestTaskStore);
  const requestTaskType = store.select(requestTaskQuery.selectRequestTaskType)();

  return {
    header: taskActionTypeToTitleMap?.[requestTaskType],
    preContentComponent: EmpVariationPeerReviewActionButtonsComponent,
    sections: [
      {
