...
export class RespondToRecommendationFlowManager extends WizardFlowManager {
  ...
  nextStepPath(currentStep: string, route: ActivatedRoute): Observable<string> {
    switch (currentStep) {
      ...
      case VirRespondToRecommendationWizardStep.UPLOAD_EVIDENCE_QUESTION:
        return this.store.select(virCommonQuery.selectOperatorResponseData(route.snapshot.params?.['key']))()
          ?.uploadEvidence === false
          ? of(`../`)
          : of(`../${VirRespondToRecommendationWizardStep.UPLOAD_EVIDENCE_FORM}`);
