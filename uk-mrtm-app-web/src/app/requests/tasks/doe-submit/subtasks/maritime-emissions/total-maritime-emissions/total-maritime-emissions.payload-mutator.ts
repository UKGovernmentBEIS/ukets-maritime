...
import { Doe, DoeTotalMaritimeEmissions } from '@mrtm/api';
...
export class TotalMaritimeEmissionsPayloadMutator extends PayloadMutator {
   ...
   */
  apply(
    currentPayload: DoeTaskPayload,
    userInput: Omit<DoeTotalMaritimeEmissions, 'supportingDocuments'> & { supportingDocuments: UploadedFile[] },
  ): Observable<DoeTaskPayload> {
    return of(
