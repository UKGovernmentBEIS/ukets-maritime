...
export const uncorrectedNonCompliancesItemFormProvider: Provider = {
  provide: TASK_FORM,
  ...
  useFactory: (formBuilder: FormBuilder, store: RequestTaskStore, route: ActivatedRoute) => {
    ...
    const item = referenceParam
      ? store
          .select(aerVerificationSubmitQuery.selectUncorrectedNonCompliances)()
          ?.uncorrectedNonCompliances?.find(({ reference }) => reference === referenceParam)
      : null;
