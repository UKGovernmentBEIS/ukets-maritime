...
const routePrefix = 'emp-variation-sent-to-peer-review';

export const empVariationWaitForPeerReviewTaskContent: RequestTaskPageContentFactory = () => {
  const store: RequestTaskStore = inject(RequestTaskStore);
  const requestTaskType = store.select(requestTaskQuery.selectRequestTaskType)();

  return {
    header: taskActionTypeToTitleMap?.[requestTaskType],
    contentComponent: WaitForPeerReviewComponent,
    sections: [
      {
