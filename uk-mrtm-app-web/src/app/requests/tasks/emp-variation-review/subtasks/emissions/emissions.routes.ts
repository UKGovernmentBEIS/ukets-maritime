...
import { canActivateEmissionsSummary } from '@requests/tasks/emp-variation-review/subtasks/emissions/emissions.guard';
...
export const EMISSIONS_ROUTES: Routes = [
  ...
  {
    path: EmissionsWizardStep.DECISION,
    title: emissionsSubTasksMap.decision.title,
    ...
    loadComponent: () =>
      import('@requests/tasks/emp-variation-review/subtasks/emissions').then(
        (c) => c.ListOfShipsVariationReviewDecisionComponent,
      ),
  },
  {
    path: EmissionsWizardStep.LIST_OF_SHIPS,
    title: emissionsSubTasksMap.title,
    canActivate: [canActivateEmissionsStep(EmissionsWizardStep.DECISION)],
    data: { breadcrumb: false },
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        ...
        canActivate: [canActivateEmissionsShipStep()],
        providers: [{ provide: HTML_DIFF, useValue: false }],
        loadComponent: () =>
