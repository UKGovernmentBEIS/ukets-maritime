...
import {
  AerRequestedChangesFlowManager,
  AerRequestedChangesQuestionPayloadMutator,
} from '@requests/tasks/aer-amend/subtasks/requested-changes';
...
export function provideAerSubmitPayloadMutators(): EnvironmentProviders {
  return makeEnvironmentProviders([
    ...
    // Required changes
    { provide: PAYLOAD_MUTATORS, multi: true, useClass: AerRequestedChangesQuestionPayloadMutator },
  ]);
}

export function provideAerSubmitTaskServices(): EnvironmentProviders {
  return makeEnvironmentProviders([
    { provide: TaskApiService, useClass: AerAmendApiService },
    { provide: TaskService, useClass: AerAmendService },
    aerCommonSubtaskStepsProvider,
    { provide: UPLOAD_SHIPS_XML_SERVICE, useClass: AerShipsXmlService },
    { provide: RETURN_FOR_AMENDS_SERVICE, useClass: AerAmendService },
  ]);
}
