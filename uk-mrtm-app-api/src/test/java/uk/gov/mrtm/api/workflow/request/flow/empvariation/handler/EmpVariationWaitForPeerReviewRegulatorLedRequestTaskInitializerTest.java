...
class EmpVariationWaitForPeerReviewRegulatorLedRequestTaskInitializerTest {

	@InjectMocks
    private EmpVariationWaitForPeerReviewRegulatorLedRequestTaskInitializer cut;
	
	@Test
	void getRequestTaskTypes() {
		assertThat(cut.getRequestTaskTypes()).containsExactly(MrtmRequestTaskType.EMP_VARIATION_REGULATOR_LED_WAIT_FOR_PEER_REVIEW);
	}
	...
	void initializePayload() {
		...
		assertThat(result).isEqualTo(EmpVariationApplicationSubmitRegulatorLedRequestTaskPayload.builder()
				.payloadType(MrtmRequestTaskPayloadType.EMP_VARIATION_WAIT_FOR_PEER_REVIEW_REGULATOR_LED_PAYLOAD)
				.emissionsMonitoringPlan(EmissionsMonitoringPlan.builder()
						.abbreviations(EmpAbbreviations.builder()
								.exist(false)
								.build())
