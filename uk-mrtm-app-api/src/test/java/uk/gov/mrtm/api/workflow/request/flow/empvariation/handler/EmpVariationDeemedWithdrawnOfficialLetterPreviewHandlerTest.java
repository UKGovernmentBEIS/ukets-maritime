...
class EmpVariationDeemedWithdrawnOfficialLetterPreviewHandlerTest {

    @InjectMocks
    private EmpVariationDeemedWithdrawnOfficialLetterPreviewHandler handler;
    ...
    void generateDocument() {
        ...
        final Request request = Request.builder().id(reqId).requestResources(
            List.of(RequestResource.builder()
                .resourceId(String.valueOf(accountId))
                .resourceType(ResourceType.ACCOUNT).build())).build();
        ...
        final TemplateParams templateParamsWithCustom = TemplateParams.builder()
            .accountParams(MrtmAccountTemplateParams.builder().name(operatorName).build()).build();
        ...
        when(fileDocumentGenerateServiceDelegator.generateFileDocument(
            MrtmDocumentTemplateType.EMP_VARIATION_DEEMED_WITHDRAWN,
    ...
    @Test
    void getTypes() {
        assertThat(handler.getTypes()).containsExactlyElementsOf(List.of(MrtmDocumentTemplateType.EMP_VARIATION_DEEMED_WITHDRAWN));
    }
