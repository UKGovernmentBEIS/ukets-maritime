...
class EmpIssuanceApplicationReviewInitializerTest {
    private static final UUID DOCUMENT_ID_1 = UUID.randomUUID();
    private static final UUID DOCUMENT_ID_2 = UUID.randomUUID();

    @InjectMocks
    private EmpIssuanceApplicationReviewInitializer initializer;

    @Test
    void initializePayload() {

        EmissionsMonitoringPlan emissionsMonitoringPlan = EmissionsMonitoringPlanFactory
                .getEmissionsMonitoringPlan(DOCUMENT_ID_1, "1234567");
        ...
        EmpIssuanceApplicationReviewRequestTaskPayload expectedRequestTaskPayload = EmpIssuanceApplicationReviewRequestTaskPayload
                .builder()
                .emissionsMonitoringPlan(emissionsMonitoringPlan)
                .empSectionsCompleted(Map.of("a", "b"))
                .empAttachments(Map.of(DOCUMENT_ID_1, "test"))
        ...
        Request request = Request.builder()
                .requestResources(
                        List.of(RequestResource.builder().resourceId(String.valueOf(accountId))
                                .resourceType(ResourceType.ACCOUNT).build()))
                .payload(payload).build();

        EmpIssuanceApplicationReviewRequestTaskPayload actualRequestTaskPayload = (EmpIssuanceApplicationReviewRequestTaskPayload) initializer
                .initializePayload(request);

        assertEquals(expectedRequestTaskPayload.getEmissionsMonitoringPlan(),
                actualRequestTaskPayload.getEmissionsMonitoringPlan());
        assertTrue(actualRequestTaskPayload.getEmpSectionsCompleted().isEmpty());
    ...
    void getRequestTaskTypes() {
        assertEquals(initializer.getRequestTaskTypes(), Set.of(MrtmRequestTaskType.EMP_ISSUANCE_APPLICATION_REVIEW));
    }

    @Test
    void getRequestTaskPayloadType() {
        assertEquals(MrtmRequestTaskPayloadType.EMP_ISSUANCE_APPLICATION_REVIEW_PAYLOAD,
