...
class NonComplianceInitialPenaltyNoticePeerDecisionActionHandlerTest {

    @InjectMocks
    private NonComplianceInitialPenaltyNoticePeerDecisionActionHandler handler;
    ...
    void process(PeerReviewDecisionType type, String requestActionType) {
        ...
        final PeerReviewDecisionRequestTaskActionPayload payload =
            PeerReviewDecisionRequestTaskActionPayload.builder()
                .payloadType(MrtmRequestTaskActionPayloadTypes.NON_COMPLIANCE_INITIAL_PENALTY_NOTICE_SUBMIT_PEER_REVIEW_DECISION_PAYLOAD)
                .decision(decision)
                .build();
        ...
        final RequestTask requestTask = RequestTask.builder()
            .id(2L)
            .request(request)
            .payload(NonComplianceInitialPenaltyNoticeRequestTaskPayload.builder().build())
            .processTaskId(processTaskId).build();
        final NonComplianceInitialPenaltyNoticeRequestTaskPayload expectedRequestTaskPayload =
        ...
        RequestTaskPayload actualRequestTaskPayload = handler.process(requestTask.getId(),
            MrtmRequestTaskActionType.NON_COMPLIANCE_INITIAL_PENALTY_NOTICE_SUBMIT_PEER_REVIEW_DECISION,
            appUser,
            payload);
        ...
        assertThat(captorValue.getPayloadType()).isEqualTo(MrtmRequestActionPayloadType.NON_COMPLIANCE_INITIAL_PENALTY_NOTICE_PEER_REVIEW_DECISION_SUBMITTED_PAYLOAD);
    ...
    public static Stream<Arguments> processScenarios() {
        return Stream.of(
            Arguments.of(PeerReviewDecisionType.AGREE, MrtmRequestActionType.NON_COMPLIANCE_INITIAL_PENALTY_NOTICE_PEER_REVIEWER_ACCEPTED),
            Arguments.of(PeerReviewDecisionType.DISAGREE, MrtmRequestActionType.NON_COMPLIANCE_INITIAL_PENALTY_NOTICE_PEER_REVIEWER_REJECTED)
        );
    }

    @Test
    void getRequestTaskTypes() {
        assertThat(handler.getTypes()).containsExactly(MrtmRequestTaskActionType.NON_COMPLIANCE_INITIAL_PENALTY_NOTICE_SUBMIT_PEER_REVIEW_DECISION);
    }
}
