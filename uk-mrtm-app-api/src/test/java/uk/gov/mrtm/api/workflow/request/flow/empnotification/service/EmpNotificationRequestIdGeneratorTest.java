...
class EmpNotificationRequestIdGeneratorTest {
	
	@InjectMocks
    private EmpNotificationRequestIdGenerator generator;
	...
	@Test
	void getTypes() {
		assertThat(generator.getTypes()).containsExactly(MrtmRequestType.EMP_NOTIFICATION);
	}
    
    @Test
    void getPrefix() {
    	assertThat(generator.getPrefix()).isEqualTo("MAN");
    }
    ...
    void generate() {
		...
		RequestType requestType = RequestType.builder().code(MrtmRequestType.EMP_NOTIFICATION).build();
		...
		when(requestTypeRepository.findByCode(MrtmRequestType.EMP_NOTIFICATION)).thenReturn(Optional.of(requestType));

		String result = generator.generate(params);
    	
    	assertThat(result).isEqualTo("MAN" + "00100" + "-" + (currentSequence + 1));
		...
		verify(requestTypeRepository).findByCode(MrtmRequestType.EMP_NOTIFICATION);
