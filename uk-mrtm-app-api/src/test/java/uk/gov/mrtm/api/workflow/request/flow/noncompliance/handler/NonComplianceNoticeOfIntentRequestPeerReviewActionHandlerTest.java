    ...
    void process() {
        ...
        final NonComplianceNoticeOfIntentRequestTaskPayload taskPayload =
            NonComplianceNoticeOfIntentRequestTaskPayload.builder()
                .payloadType(MrtmRequestTaskPayloadType.NON_COMPLIANCE_NOTICE_OF_INTENT_APPLICATION_PEER_REVIEW_PAYLOAD)
                .build();
        final NonComplianceNoticeOfIntentRequestTaskPayload expectedTaskPayload =
            NonComplianceNoticeOfIntentRequestTaskPayload.builder()
                .payloadType(MrtmRequestTaskPayloadType.NON_COMPLIANCE_NOTICE_OF_INTENT_APPLICATION_PEER_REVIEW_PAYLOAD)
                .build();
        ...
        RequestTaskPayload actualTaskPayload = handler.process(
            requestTaskId,
            MrtmRequestTaskActionType.NON_COMPLIANCE_NOTICE_OF_INTENT_REQUEST_PEER_REVIEW,
            appUser,
            taskActionPayload);
        ...
        verify(validator, times(1))
            .validateNoticeOfIntentPeerReview(taskPayload, requestTask, selectedPeerReviewer, appUser);
        verify(applyService, times(1))
            .saveRequestPeerReviewAction(requestTask, selectedPeerReviewer, regulatorReviewer);
        verify(requestService, times(1)).addActionToRequest(
            requestTask.getRequest(),
            null,
            MrtmRequestActionType.NON_COMPLIANCE_NOTICE_OF_INTENT_PEER_REVIEW_REQUESTED,
            appUser.getUserId()
        );
        verify(workflowService, times(1)).completeTask(
            ...
            Map.of(
                ...
                MrtmRequestCustomContext.NON_COMPLIANCE_NOTICE_OF_INTENT.getCode()
            )
        );
