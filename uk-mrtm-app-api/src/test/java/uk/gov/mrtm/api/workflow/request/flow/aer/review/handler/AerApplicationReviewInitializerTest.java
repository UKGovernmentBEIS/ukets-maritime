...
class AerApplicationReviewInitializerTest {
    ...
    void initializePayload_when_verification_report_exists() {
        ...
        verify(aerReviewMapper, times(1))
                .toAerApplicationReviewRequestTaskPayload(requestPayload, MrtmRequestTaskPayloadType.AER_APPLICATION_REVIEW_PAYLOAD, reportingYear);
    }

    @Test
    void getRequestTaskTypes() {
        assertThat(initializer.getRequestTaskTypes()).containsOnly(MrtmRequestTaskType.AER_APPLICATION_REVIEW);
    }

    @Test
    void getRequestTaskPayloadType() {
        assertEquals(MrtmRequestTaskPayloadType.AER_APPLICATION_REVIEW_PAYLOAD, initializer.getRequestTaskPayloadType());
    }
}
