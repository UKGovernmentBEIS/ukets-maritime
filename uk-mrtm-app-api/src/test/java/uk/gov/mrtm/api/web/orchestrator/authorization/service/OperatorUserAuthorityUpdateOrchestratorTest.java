...
class OperatorUserAuthorityUpdateOrchestratorTest {
    ...
    void updateOperatorUserByAccountAndId() {
        ...
        verify(publisher).publishEvent(AccountContactsRegistryEvent.builder().accountIds(Set.of(accountId1, accountId2)).build());
        verify(authorityRepository).findByUserIdAndStatus(userId, AuthorityStatus.ACTIVE);

        verifyNoMoreInteractions(operatorUserManagementService, publisher);
    }
