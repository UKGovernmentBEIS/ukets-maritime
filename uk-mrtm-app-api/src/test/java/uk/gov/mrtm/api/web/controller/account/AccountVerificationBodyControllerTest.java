...
class AccountVerificationBodyControllerTest {
    ...
    @Test
    void unappointAccountAppointedToVerificationBody() throws Exception {
        AppUser user = AppUser.builder().build();
        Long accountId = 1L;
        ...
        mockMvc.perform(
                        MockMvcRequestBuilders
                                .delete(CONTROLLER_PATH + "/" + accountId + "/unappoint-verification-body")
                                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNoContent());

        verify(accountVerificationBodyUnappointService)
                .unappointAccountAppointedToVerificationBody(accountId);
    }

    @Test
    void unappointAccountAppointedToVerificationBody_forbidden() throws Exception {
        AppUser user = AppUser.builder().build();
        Long accountId = 1L;
        ...
        doThrow(new BusinessException(ErrorCode.FORBIDDEN))
                .when(appUserAuthorizationService)
                .authorize(user, "unappointVerificationBodyFromAccount", String.valueOf(accountId), null, null);
        ...
        verify(accountVerificationBodyUnappointService, never())
                .unappointAccountAppointedToVerificationBody(accountId);
    }
}
