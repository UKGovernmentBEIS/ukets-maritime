...
class EmpVariationApplicationPeerReviewInitializerTest {

	@InjectMocks
    private EmpVariationApplicationPeerReviewInitializer initializer;
    ...
    void initializePayload() {
        ...
        EmpVariationRequestPayload requestPayload = EmpVariationRequestPayload.builder()
            ...
            .build();
        Request request = Request.builder().id(requestTaskId).payload(requestPayload).build();

        RequestTaskPayload requestTaskPayload = initializer.initializePayload(request);

        assertThat(requestTaskPayload.getPayloadType()).isEqualTo(MrtmRequestTaskPayloadType.EMP_VARIATION_APPLICATION_PEER_REVIEW_PAYLOAD);
        assertThat(requestTaskPayload).isInstanceOf(EmpVariationApplicationReviewRequestTaskPayload.class);
    }

    @Test
    void getRequestTaskTypes() {
        assertThat(initializer.getRequestTaskTypes()).containsExactly(MrtmRequestTaskType.EMP_VARIATION_APPLICATION_PEER_REVIEW);
    }
