...
class FormValidationViolationMapperTest {
    ...
    @Test
    void formatInvalidFormatMessage_invalidLocalDate_returnsHumanReadableMessage() throws Exception {
        InvalidFormatException exception = invalidLocalDateException(
            """
                {"delegatedResponsibilityUsed":true,"registeredOwners":[{"name":"owner","companyImoNumber":"1234567",\
                "contactName":"contact","email":"e@example.com","agreementDate":"2025-11-4g",\
                "ships":[{"shipImoNumber":"7654321","name":"ship"}]}]}
                """);

        String message = FormValidationViolationMapper.formatInvalidFormatMessage(exception);

        assertThat(message).isEqualTo("agreementDate must be a valid date in format yyyy-MM-dd");
    }

    @Test
    void fromHttpMessageNotReadable_invalidLocalDate_returnsIndexedPathAndHumanMessage() throws Exception {
        InvalidFormatException invalidFormatException = invalidLocalDateException(
            """
                {"delegatedResponsibilityUsed":true,"registeredOwners":[\
                {"name":"first","companyImoNumber":"1234567","contactName":"contact","email":"e@example.com",\
                "agreementDate":"2025-01-01","ships":[{"shipImoNumber":"7654321","name":"ship"}]},\
                {"name":"second","companyImoNumber":"2345678","contactName":"contact","email":"e@example.com",\
                "agreementDate":"2025-11-4g","ships":[{"shipImoNumber":"8765432","name":"ship2"}]}]}
                """);

        HttpMessageNotReadableException exception =
            new HttpMessageNotReadableException("Invalid date", invalidFormatException);

        var violations = FormValidationViolationMapper.fromHttpMessageNotReadable(exception);

        assertThat(violations).isPresent();
        assertThat(violations.get()).hasSize(1);
        assertThat(violations.get()[0].getFieldName()).isEqualTo("registeredOwners[1].agreementDate");
        assertThat(violations.get()[0].getMessage())
            .isEqualTo("agreementDate must be a valid date in format yyyy-MM-dd");
    }

    @Test
    void fromHttpMessageNotReadable_unmappableCause_returnsEmpty() {
        HttpMessageNotReadableException exception =
            new HttpMessageNotReadableException("Malformed JSON", new IllegalArgumentException("unexpected"));
    ...
    private static InvalidFormatException invalidLocalDateException(String json) throws Exception {
        try {
            OBJECT_MAPPER.readValue(json, ExternalEmpDelegatedResponsibility.class);
        } catch (InvalidFormatException ex) {
            return ex;
        }
