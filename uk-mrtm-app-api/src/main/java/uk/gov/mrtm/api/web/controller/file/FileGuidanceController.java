...
@RestController
@RequestMapping(path = "/v1.0/file-guidance")
@RequiredArgsConstructor
@Tag(name = "File Guidance")
@SecurityRequirements
public class FileGuidanceController {

    private final FileGuidanceTokenService fileGuidanceTokenService;


    @GetMapping(path = "/{token}")
    @Operation(summary = "Get the file guidance resource for the provided file token")
    ...
    public ResponseEntity<Resource> getFileGuidance(
            @PathVariable("token") @Parameter(description = "The file guidance token", required = true) @NotEmpty String token) {

        final FileDTO file = fileGuidanceTokenService.getFileDTOByToken(token);
