...
public class ExternalAerDataGapsMethodologies {
    ...
    @Schema(nullable = true, description = "Indicates if the data gap method has already been approved by the regulator. Required only when 'methodRequired' is true, otherwise must be omitted")
    private Boolean methodApproved;

    @Schema(nullable = true, description = "Indicates if the method used was conservative. Required only when 'methodApproved' is false, otherwise must be omitted")
    ...
    @Schema(nullable = true, description = "Indicates if method led to a material misstatement. Required only when 'methodApproved' is false, otherwise must be omitted")
