...
public class FormValidationViolationMapper {
    ...
    static String formatInvalidFormatMessage(InvalidFormatException exception) {
        ...
        if (LocalDate.class.equals(targetType)) {
            return extractLeafFieldName(exception.getPath()) + " must be a valid date in format yyyy-MM-dd";
        }

        return StringUtils.defaultIfBlank(exception.getOriginalMessage(), "Invalid value");
    }

    static String extractLeafFieldName(List<JsonMappingException.Reference> path) {
        if (path == null || path.isEmpty()) {
            return "value";
        }
        for (int index = path.size() - 1; index >= 0; index--) {
            String fieldName = path.get(index).getFieldName();
