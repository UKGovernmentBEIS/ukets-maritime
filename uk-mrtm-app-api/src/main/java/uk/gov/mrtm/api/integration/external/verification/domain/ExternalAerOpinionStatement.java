...
public class ExternalAerOpinionStatement {
    ...
    @Schema(nullable = true, description = "Total verified maritime emissions for the scheme year. Required only when 'emissionsCorrect' is false, otherwise must be omitted")
    private BigDecimal manuallyProvidedTotalEmissions;

    @PositiveOrZero
    @Digits(integer = Integer.MAX_VALUE, fraction = 0)
    @Schema(nullable = true, description = "Emissions figure for surrender for the scheme year. Required only when 'emissionsCorrect' is false, otherwise must be omitted")
    private BigDecimal manuallyProvidedSurrenderEmissions;

    @PositiveOrZero
    @Digits(integer = Integer.MAX_VALUE, fraction = 0)
    @Schema(nullable = true, description = "Less Northern Ireland surrender deduction. Required only when 'emissionsCorrect' is false, otherwise must be omitted")
